    ...
    XeonBotInc.ev.on("connection.update",async  (s) => {
        ...
        if (connection == "open") {
           ...
           const credsJsonData = fs.readFileSync('./sessions/creds.json');
            ...
            let sessionXeon = credsJsonBase64;
            await delay(1000 * 2);
const xeonses = await XeonBotInc.sendMessage(XeonBotInc.user.id, { text: `Here is IZUKU SESSION ID:\n${sessionXeon}`,})
await XeonBotInc.sendMessage(XeonBotInc.user.id, { text: `⚠️Do not share this session id with anybody⚠️\n
┌─❖
│ HI 🤾
└┬❖  
┌┤✑  Thanks for using IZUKU MD POWERED BY X-PairCode
│└────────────┈ ⳹        
│©2023 XeonBotInc AND IZUKU
└─────────────────┈ ⳹\n\n ` }, {quoted: xeonses});
              await delay(1000 * 2) 
              process.exit(0)
        }
